...
for key in data_dict.keys():
    if data_dict[key]["salary"] != 'NaN' and data_dict[key]["bonus"] != 'NaN' and data_dict[key]["poi"] == True:
        tmp.append({"key": key, "salary": data_dict[key]["salary"], "bonus": data_dict[key]["bonus"],
                    "email_address": data_dict[key]["email_address"]})
...
for email in specific_emails:
    try:
        ...
        for path_str in list_messages:
            ...
            to_addresses = get_To_Emails(path)
...
for key in data_dict:
    ...
    data_point["fraction_from_poi"] = compute_Fraction(data_point["from_this_person_to_poi"],
                                                      data_point["from_messages"])
    data_point["fraction_to_poi"] = compute_Fraction(data_point["from_poi_to_this_person"], data_point["to_messages"])
...
### Store to new_dataset for easy export below.
...
dump_classifier_and_data(clf, new_dataset, features_list)